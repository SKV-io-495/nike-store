...
export async function getAllProducts(
  ...
  // --- ADDED LOGGING START ---
  console.log("[RUNTIME LOG] Attempting to fetch all products with filters:", JSON.stringify(filters));
  try {
  // --- ADDED LOGGING END ---
    ...
    // --- ADDED LOGGING START ---
    console.log(`[RUNTIME LOG] Successfully fetched ${productsOut.length} products out of ${totalCount} total.`);
    return { products: productsOut, totalCount };
  } catch (error) {
    console.error("[RUNTIME CRASH LOG] Database query in getAllProducts failed:", error);
    // Re-throwing the error to ensure the process crashes and we see the log.
    throw new Error("Failed to fetch products due to a database error.");
  }
  // --- ADDED LOGGING END ---
}
